...
exports.handler = async function(event, context) {
  ...
  // Strip ZIP code from address2 - ATTOM works better without it
  const address2Clean = address2.replace(/\s*\d{5}(-\d{4})?\s*$/, '').trim();
  
  const path = '/propertyapi/v1.0.0/property/basicprofile?address1=' + 
    encodeURIComponent(address1) + '&address2=' + encodeURIComponent(address2Clean);
